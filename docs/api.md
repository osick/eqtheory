...
|---|---|
| `eqtheory.config` | `settings`, `Settings`, `configure`, `load`, `reload`, `describe`, `config_file` — defaults → TOML file → `EQTHEORY_*` env → programmatic |
...
| `eqtheory.lean.certs` | `prelude`, `true_code`, `false_code`, `false_table_code`, `false_affine_code`, `false_nat_residue_code`, `superposition_code`, `egraph_proof_code`, `seeded_grind_bodies`, `lemma_haves`, `render_chain`; `style="standalone"` (default) or `"judge"` |
| `eqtheory.lean.check` | `configure`, `find_lean`, `LeanConfig`, `compile_certificate(code, cfg)`, `make_judge(cfg)`, `verdict_of`; `problem_module` (judge style) |
...
| `eqtheory.cli` | `eqtheory solve / prove / model / egraph / viz-proof / cert / config` |
